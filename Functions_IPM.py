...
def run_year_n(ic):
    pop = odeint(func = dPop_n, y0 = ic, t = t_growing)
    return pop
...
# Fungicide ODEs
def dPop_f(x, t,omega,theta,delta):
...
def run_year_f(ic, omega = omega_base, theta = theta_base, delta = delta_base):
    pop = odeint(func = dPop_f, y0 = ic, t = t_growing,args=(omega,theta,delta))
    return pop
...
###########################################
# IPM ODEs
def dPop_i(x,t,r_beta,days_late,biocontrol):
...
# Run IPM for a year
def run_year_i(input_ic,r_beta,days_late,prop_debris,cleared_debris,biocontrol_dates):
    ic = 1*input_ic
    ...
    pop = odeint(func = dPop_i, y0 = ic, t = t_growing, args = (r_beta,days_late,this_biocontrol))
    return pop
...
def dPop_mix2(x,t,r_beta,delay,biocontrol,prop_i,omega,theta,delta):
...
def run_year_mix2(input_ic,r_beta,days_late,prop_debris,cleared_debris,biocontrol_dates,prop_i,omega=omega_base,theta=theta_base,delta=delta_base):
    ...
    pop0 = odeint(func = dPop_mix2, y0 = ic0, t = t_0, args = (r_beta,delay,this_biocontrol,prop_i,omega,theta,delta))
    
    # Run second period after IPM fields are sown
    ic1 = 1*pop0[-1]
    ic1[:6] = 1*ic[:6]
    t_1 = np.arange(Temerge+delay,T87,1)
    pop1 = odeint(func = dPop_mix2, y0 = ic1, t = t_1, args = (r_beta,delay,this_biocontrol,prop_i,omega,theta,delta))
